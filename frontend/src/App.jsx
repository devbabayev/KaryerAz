...
const CVView = ({ onAnalysisComplete }) => {
  ...
  const SKILL_DB = ['React','Vue','Angular','Javascript','Typescript','Python','Java','C++','C#','Figma','Node.js','AWS','Docker','Kubernetes','SQL','NoSQL','HTML','CSS','Next.js','Tailwind','Git','Linux','AI','ML','Django','Flask','PostgreSQL','MongoDB','Redis','Flutter','Swift','Kotlin','Go','PHP','GraphQL','Azure'];
  ...
  const handleFileSelect = async (file) => {
    ...
    try {
      ...
      setResumeText(text || '');
      if (!text) setError('Fayl oxuna bilmədi. Mətni əl ilə yapışdırın.');
    } catch { setError('Fayl xətası. Mətni əl ilə yapışdırın.'); }
  };

  const handleAnalyze = async () => {
    const text = resumeText.trim();
    if (!text) { setError('Zəhmət olmasa CV mətnini yapışdırın'); return; }
  ...
  const reset = () => { setStep(QUIZ_STEP.UPLOAD);setResumeText('');setFileName('');setError('');setResumeAnalysis(null);setGapAnalysis(null);setQuiz([]);setScores(null);setRoadmap(null);setCurrentQ(0);setUserAnswers({});setSelectedOpt(null); };
  ...
  if (step === QUIZ_STEP.UPLOAD) return (
    <div className="view-content fade-in">
      ...
      <div className="anim-fadeup anim-delay-1">
        <input type="file" ref={fileRef} style={{display:'none'}} accept=".pdf,.txt,.doc" onChange={e=>{ if(e.target.files[0]) handleFileSelect(e.target.files[0]); }}/>
        <div className="upload-box-dashed" onClick={()=>fileRef.current.click()} style={{marginBottom:'20px'}}>
          <Cloud size={48} className="upload-cloud"/>
          <p className="upload-title">{fileName || 'CV-ni yükləyin'}</p>
          <p className="upload-subtitle">{fileName ? 'Fərqli fayl seçmək üçün klikləyin' : 'PDF, TXT formatları dəstəklənir'}</p>
        </div>
        <div style={{position:'relative',marginBottom:'20px'}}>
          <div style={{position:'absolute',top:'50%',left:0,right:0,height:'1px',background:'var(--glass-border)'}}/>
          <span style={{position:'relative',background:'var(--bg-color)',padding:'0 12px',color:'var(--text-secondary)',fontSize:'13px',display:'block',width:'fit-content',margin:'0 auto'}}>və ya</span>
        </div>
        <textarea value={resumeText} onChange={e=>setResumeText(e.target.value)} placeholder={"Ad Soyad\nPython Developer\n\nBacarıqlar: Python, Django, SQL..."} rows={7}
        ...
        <PrimaryBtn onClick={handleAnalyze}>🔍 CV-ni Analiz Et</PrimaryBtn>
      </div>
    </div>
  );
